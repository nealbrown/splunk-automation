"""Configure Splunk Deployment Server via REST API"""
...
reload_deploymentserver_app   = typer.Typer(pretty_exceptions_show_locals=False,
    help="Reload deployment server.")
serverclass_app               = typer.Typer(pretty_exceptions_show_locals=False,
    help="Retrieve and manage serverclasses.")
deploymentapps_app            = typer.Typer(pretty_exceptions_show_locals=False,
    help="Manage deploymentapps.")
...
# Note that instead of using typing.List we expect singular arguments
# unless we are pulling from the toml config file

@app.callback(invoke_without_command=True)
def main(
    ctx: typer.Context,
    password: Annotated[
        str,typer.Option(envvar="SPLUNK_PASSWORD",
        prompt="SPLUNK_PASSWORD not set, enter admin password",
        hide_input=True)
        ],
    host: Annotated[
    ...
    Then each subcommand can operate on applications 
    or serverclasses as specified in their individual descriptions.
    ...
    r = requests.get("https://" + host + ":8089" + "/services/auth/login",
        data={'username':user,'password':password}, verify=False, timeout=30)
    global SESSION_KEY # We need to set this as a global variable to use in the subcommands
...
def reload_deploymentserver(
    ...
    """
    Reload Deployment Server- not necessary in most cases 
    because the API call itself will trigger a reload.
...
def create_serverclass(
    ...
    r = requests.post("https://" + host + ":8089" + "/services/deployment/server/serverclasses",
        headers = { 'Authorization': ('Splunk %s' %SESSION_KEY)},
        data={"name": serverclass}, verify=False, timeout=30)
...
def create_all_serverclasses(
    ...
    for pkg in apps['app']:
        ...
        # We get the clients to add to the serverclass
        # from the toml file with the index as the value
        ...
        if name:
            for n in name[1:]: # We have to skip the top level "serverclasses" element
                print("Serverclass Created: " +
                " ".join(t.nodeValue for t in n.childNodes if t.nodeType == t.TEXT_NODE))
        else:
...
def add_host_to_serverclass(
    ...
    allowlist: Annotated[
        str, typer.Option(help="Whitelist or Blacklist with Int e.g. whitelist.0")] =
         DEFAULT_ALLOW_LIST,
    host: Annotated[
        str, typer.Option(envvar="SPLUNK_HOST")] = DEFAULT_SPLUNK_HOST,
    ):
    ...
    # python main.py serverclass add-host-to-serverclass 
    #   --serverclass Splunk_TA_nix --client splunk-hf-1
    # python main.py serverclass add-host-to-serverclass 
    #   --serverclass Splunk_TA_nix --client "splunk-hf-*" --list whitelist.1
    ...
    """
    r = requests.post("https://" + host + ":8089" +
        "/servicesNS/nobody/search/deployment/server/serverclasses/" + serverclass,
        headers = { 'Authorization': ('Splunk %s' %SESSION_KEY)},
        data={ allowlist : client }, verify=False, timeout=30)
...
def add_hosts_to_serverclasses(
    ...
    for pkg in apps['app']:
        ...
        r = requests.post("https://" + host + ":8089" +
            "/services/deployment/server/serverclasses/" + pkg,
            headers = { 'Authorization': ('Splunk %s' %SESSION_KEY)},
            data = allowlists, verify = False, timeout=30)
        if debug:
            print(r.request.url)
        ...
        if name:
            for n in name[1:]: # We have to skip the top level "serverclasses" element
                print(f"Host(s) {allowlists} [bold]Added to Serverclass[/bold]: " +
                " ".join(t.nodeValue for t in n.childNodes if t.nodeType == t.TEXT_NODE))
...
def add_serverclass_to_app(
    ...
    # python main.py deploymentapps add-serverclass-to-app 
    #   --application Splunk_TA_nix --serverclass Splunk_TA_nix
    """
    r = requests.post("https://" + host + ":8089" +
        "/servicesNS/nobody/system/deployment/server/applications/" + application,
        headers = { 'Authorization': ('Splunk %s' %SESSION_KEY)},
        data={"serverclass": serverclass}, verify=False, timeout=30)
...
def add_all_serverclasses_to_app(
    ...
    for pkg in apps['app']:
        ...
        r = requests.post("https://" + host + ":8089" +
            "/servicesNS/nobody/system/deployment/server/applications/" + pkg,
            headers = { 'Authorization': ('Splunk %s' %SESSION_KEY)},
            data={"serverclass": {pkg}}, verify=False, timeout=30)
        r.encoding = r.apparent_encoding
        if r.status_code == requests.codes.ok:
            if debug:
                print(f"Debug Output: XML of DeploymentApps: {r.text}")
